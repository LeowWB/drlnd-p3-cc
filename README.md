...
The observation space consists of 8 variables corresponding to the position and velocity of the ball and racket. Each agent receives its own, local observation. Two continuous actions are available for each agent, corresponding to movement toward (or away from) the net, and jumping. 
...
6. Place the file in the DRLND GitHub repository (from step 3), in the `p3_collab-compet/` folder, and unzip (or decompress) the file. 

7. Copy all files from the `src/` directory of this repository, into the `p3_collab-compet/` folder of the DRLND GitHub repository.
...
1. Navigate to the `p3_collab-compet/` directory of the DRLND repository.
2. Execute `jupyter notebook` and open `Tennis.ipynb`.
